...
void newmm_rking_sp(float **A, int transa, float **B, int transb, float **C, int num_rows, int num_links, int num_cols,
    ...
    float tmp11, tmp12, tmp13, tmp21, tmp22, tmp23, tmp31, tmp32, tmp33;
    float *tmpA1, *tmpA2, *tmpA3;
    float *tmpB1, *tmpB2, *tmpB3, *tmpB4, *tmpB5;
    float *tmpC1, *tmpC2, *tmpC3;
    ...
    if (beta == 0.0) {
        for (i = 0; i < num_rows; ++i)
            for (j = 0; j < num_cols; ++j) C[i][j] = 0.0;
    } else if (beta != 1.0)
        for (i = 0; i < num_rows; i++)
            for (j = 0; j < num_cols; j++) C[i][j] *= beta;

    if (!transa) {
        if (alpha != 1.0) {
            for (i = 0; i < num_rows; ++i)
                for (k = 0; k < num_links; ++k) A[i][k] *= alpha;
        }
        if (!transb) {
            /* C = alpha A * B + beta C */
        ...
        if (alpha != 1.0) {
            for (k = 0; k < num_links; ++k)
                for (i = 0; i < num_rows; ++i) A[k][i] *= alpha;
        }
