...
int DPD::dot23_mp(dpdfile2<float> *T, dpdbuf4<float> *I, dpdfile2<double> *Z, int transt, int transz, float alpha, double beta) {
    ...
    for (h = 0; h < nirreps; h++) {
        buf4_mat_irrep_init_sp(I, h);
        buf4_mat_irrep_rd_sp(I, h);
        ...
        for (Gp = 0; Gp < nirreps; Gp++) {
            ...
            if (T->params->ppi[Gq] && T->params->qpi[Gr]) X = dpd_block_matrix_sp(T->params->ppi[Gq], T->params->qpi[Gr]);
            ...
            for (p = 0; p < Z->params->ppi[Gp]; p++) {
                ...
                for (s = 0; s < Z->params->qpi[Gs]; s++) {
                    ...
                    value = dot_block_sp(T->matrix[Tblock], X, T->params->ppi[Gq], T->params->qpi[Gr], alpha);
                    value_dp = static_cast<double>(value);
                    Z->matrix[Zblock][p][s] += value_dp;
                }
            }
            if (T->params->ppi[Gq] && T->params->qpi[Gr]) free_dpd_block_sp(X, T->params->ppi[Gq], T->params->qpi[Gr]);
        }
