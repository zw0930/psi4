...
int DPD::dot14(dpdfile2<double> *T, dpdbuf4<double> *I, dpdfile2<double> *Z, int transt, int transz, double alpha, double beta) {
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (Gq = 0; Gq < nirreps; Gq++) {
            ...
            if (T->params->ppi[Gp] && T->params->qpi[Gs]) X = dpd_block_matrix(T->params->ppi[Gp], T->params->qpi[Gs]);
            ...
            for (q = 0; q < Z->params->ppi[Gq]; q++) {
                ...
                for (r = 0; r < Z->params->qpi[Gr]; r++) {
                    ...
                    value = dot_block(T->matrix[Tblock], X, T->params->ppi[Gp], T->params->qpi[Gs], alpha);

                    Z->matrix[Zblock][q][r] += value;
                }
            }
            if (T->params->ppi[Gp] && T->params->qpi[Gs]) free_dpd_block(X, T->params->ppi[Gp], T->params->qpi[Gs]);
        }
