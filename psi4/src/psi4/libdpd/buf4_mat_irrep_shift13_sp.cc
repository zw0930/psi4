...
int DPD::buf4_mat_irrep_shift13_sp(dpdbuf4<float> *Buf, int buf_block) {
    ...
    Buf->shift.matrix[buf_block] = (float ***)malloc(nirreps * sizeof(float **));
    for (h = 0; h < nirreps; h++)
        Buf->shift.matrix[buf_block][h] = ((!Buf->shift.rowtot[buf_block][h])
                                               ? nullptr
                                               : (float **)malloc(Buf->shift.rowtot[buf_block][h] * sizeof(float *)));
    ...
    for (h = 1; h < nirreps; h++)
        dataoff[h] =
            dataoff[h - 1] + ((long)Buf->shift.rowtot[buf_block][h - 1]) * ((long)Buf->shift.coltot[buf_block][h - 1]);
    ...
    for (h = 0; h < Buf->params->nirreps; h++) {
        for (i = 0; (i < Buf->shift.rowtot[buf_block][h]) && Buf->shift.coltot[buf_block][h]; i++, count[h]++) {
