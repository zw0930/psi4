...
**
*/
int DPD::buf4_scm_sp(dpdbuf4<float> *InBuf, float alpha) {
    int pq;
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        if (incore) {
            ...
            if (!new_buf4) buf4_mat_irrep_rd_sp(InBuf, h);
            ...
            buf4_mat_irrep_row_init_sp(InBuf, h);

            for (pq = 0; pq < InBuf->params->rowtot[h]; pq++) {
                if (!new_buf4) buf4_mat_irrep_row_rd_sp(InBuf, h, pq);
            ...
            buf4_mat_irrep_row_close_sp(InBuf, h);
        }
    }
