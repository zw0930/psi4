...
int DPD::contract422_mp(dpdbuf4<float> *X, dpdfile2<float> *Y, dpdfile2<double> *Z, int trans_Y, int trans_Z, float alpha, double beta) {
    ...
    for (row = 0; row < X->params->rowtot[hxbuf]; row++) {
        ...
        for (Gr = 0; Gr < nirreps; Gr++) {
            ...
            if (X->params->rpi[Gr] && X->params->spi[Gs]) {
                if (trans_Y) {
                    TMP = dpd_block_matrix_sp(X->params->spi[Gs], X->params->rpi[Gr]);
                } else {
                    TMP = dpd_block_matrix_sp(X->params->rpi[Gr], X->params->spi[Gs]);
                }
            }
            ...
            if (trans_Y) {
                value += dot_block_sp(TMP, Y->matrix[Gs], X->params->spi[Gs], X->params->rpi[Gr], alpha);
            } else {
                value += dot_block_sp(TMP, Y->matrix[Gr], X->params->rpi[Gr], X->params->spi[Gs], alpha);
            }

            if (X->params->rpi[Gr] && X->params->spi[Gs]) {
                if (trans_Y) {
                    free_dpd_block_sp(TMP, X->params->spi[Gs], X->params->rpi[Gr]);
                } else {
                    free_dpd_block_sp(TMP, X->params->rpi[Gr], X->params->spi[Gs]);
                }
            }
        }
        // Cast the value into double-precision
        value_double = static_cast<double>(value);
        if (trans_Z)
            Z->matrix[qsym][Q][P] = beta * Z->matrix[qsym][Q][P] + value_double;
        else
            Z->matrix[psym][P][Q] = beta * Z->matrix[psym][P][Q] + value_double;
    }

    buf4_mat_irrep_close_sp(X, GZ);
