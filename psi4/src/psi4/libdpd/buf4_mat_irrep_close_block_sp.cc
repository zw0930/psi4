...
int DPD::buf4_mat_irrep_close_block_sp(dpdbuf4<float> *Buf, int irrep, int num_pq) {
    ...
    if (num_pq && Buf->params->coltot[irrep ^ all_buf_irrep])
        free_dpd_block_sp(Buf->matrix[irrep], num_pq, Buf->params->coltot[irrep ^ all_buf_irrep]);
