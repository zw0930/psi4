...
int DPD::dot24_mp(dpdfile2<float> *T, dpdbuf4<float> *I, dpdfile2<double> *Z, int transt, int transz, float alpha, double beta) {
     ...
     */
            ...
            if (T->params->ppi[Gq] && T->params->qpi[Gs]) X = dpd_block_matrix_sp(T->params->ppi[Gq], T->params->qpi[Gs]);
            ...
            for (p = 0; p < Z->params->ppi[Gp]; p++) {
                ...
                for (r = 0; r < Z->params->qpi[Gr]; r++) {
                    ...
                    value = dot_block_sp(T->matrix[Tblock], X, T->params->ppi[Gq], T->params->qpi[Gs], alpha);
                    value_dp = static_cast<double>(value);
                    Z->matrix[Zblock][p][r] += value_dp;
                }
            }
