...
int DPD::buf4_dirprd_sp(dpdbuf4<float> *BufA, dpdbuf4<float> *BufB) {
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        dirprd_block_sp(BufA->matrix[h], BufB->matrix[h], BufA->params->rowtot[h], BufA->params->coltot[h ^ my_irrep]);

        buf4_mat_irrep_wrt_sp(BufB, h);
