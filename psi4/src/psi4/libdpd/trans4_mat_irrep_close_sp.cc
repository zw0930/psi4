...
int DPD::trans4_mat_irrep_close_sp(dpdtrans4<float> *Trans, int irrep) {
    ...
    if (Trans->shift.shift_type) {
        for (h = 0; h < nirreps; h++)
            if (Trans->shift.rowtot[irrep][h]) free(Trans->shift.matrix[irrep][h]);
        free(Trans->shift.matrix[irrep]);
        Trans->shift.shift_type = 0;
    }

    if (size) free_dpd_block_sp(Trans->matrix[irrep], rowtot, coltot);
