 ...
 */
int DPD::contract424_mp(dpdbuf4<float> *X, dpdfile2<float> *Y, dpdbuf4<double> *Z, int sum_X, int sum_Y, int Ztrans, float alpha, double beta) {
    ...
    for (hxbuf = 0; hxbuf < nirreps; hxbuf++) {
        ...
        if (incore) {
            ...
            if (rking)
                ...
                TMP = dpd_block_matrix_sp(numrows[Hz],numcols[Hz]);
         ...
         outfile->Printf("numrows %d numlinks %d numcols %d\n",numrows[Hz],numlinks[Hy],numcols[Hz]); */
                    newmm_rking_sp(Xmat[Hx], Xtrans, Y->matrix[Hy], Ytrans, TMP, numrows[Hz], numlinks[Hy ^ symlink],
                                numcols[Hz], alpha, 0);
                for (row = 0; row < numrows[Hz]; row++){
                     for (col = 0; col < numcols[Hz]; col++){
                          Zmat[Hz][row][col] = beta * Zmat[Hz][row][col] + static_cast<double>(TMP[row][col]);
                     }
                }
                free_dpd_block_sp(TMP,numrows[Hz],numcols[Hz]);
                }
            else
                for (Hz = 0; Hz < nirreps; Hz++) {
                    TMP = dpd_block_matrix_sp(numrows[Hz],numcols[Hz]);
            ...
            for (pq = 0; pq < Z->params->rowtot[hzbuf]; pq++) {
                ...
                for (Gr = 0; Gr < nirreps; Gr++) {
                    ...
                    TMP = dpd_block_matrix_sp(rowx,colz);

                    if (rowx && colx && colz) {
                        C_SGEMM('n', Ytrans ? 't' : 'n', rowx, colz, colx, alpha, &(X->matrix[hxbuf][0][xcount]), colx,
                                &(Y->matrix[Ytrans ? GsZ : GsX][0][0]), Ytrans ? colx : colz, 0,
                                &(TMP[0][0]), colz);
                    }
                    for (row = 0; row < rowx; row++){
                         for (col = 0; col < colz; col++){
                                Z->matrix[hzbuf][row][zcount+col] = beta * Z->matrix[hzbuf][row][zcount+col] + static_cast<double>(TMP[row][col]); 
                         }
                    }

                    xcount += rowx * colx;
                    zcount += rowz * colz;
                    free_dpd_block_sp(TMP,rowx,colz);
                }
