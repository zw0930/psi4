...
int DPD::dot14_mp(dpdfile2<float> *T, dpdbuf4<float> *I, dpdfile2<double> *Z, int transt, int transz, float alpha, double beta) {
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (Gq = 0; Gq < nirreps; Gq++) {
            ...
            if (T->params->ppi[Gp] && T->params->qpi[Gs]) X = dpd_block_matrix_sp(T->params->ppi[Gp], T->params->qpi[Gs]);
            ...
            for (q = 0; q < Z->params->ppi[Gq]; q++) {
                ...
                for (r = 0; r < Z->params->qpi[Gr]; r++) {
                    ...
                    value = dot_block_sp(T->matrix[Tblock], X, T->params->ppi[Gp], T->params->qpi[Gs], alpha);
                    value_dp = static_cast<double>(value);
                    Z->matrix[Zblock][q][r] +=  value_dp;
                }
            }
            if (T->params->ppi[Gp] && T->params->qpi[Gs]) free_dpd_block_sp(X, T->params->ppi[Gp], T->params->qpi[Gs]);
        }
