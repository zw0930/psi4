...
int DPD::file4_mat_irrep_row_wrt_sp(dpdfile4_sp *File, int irrep, int row) {
    ...
    if (coltot) {
        ...
        for (; row > seek_block; row -= seek_block)
            row_ptr = psio_get_address(row_ptr, sizeof(float) * seek_block * coltot);
        row_ptr = psio_get_address(row_ptr, sizeof(float) * row * coltot);
    }

    if (coltot)
        psio_write(File->filenum, File->label, (char *)File->matrix[irrep][0], coltot * sizeof(float), row_ptr,
                   &next_address);
