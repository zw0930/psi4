...
int DPD::dot13_mp(dpdfile2<float> *T, dpdbuf4<float> *I, dpdfile2<double> *Z, int transt, int transz, float alpha, double beta) {
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (Gq = 0; Gq < nirreps; Gq++) {
            ...
            if (T->params->ppi[Gp] && T->params->qpi[Gr]) X = dpd_block_matrix_sp(T->params->ppi[Gp], T->params->qpi[Gr]);
            ...
            for (q = 0; q < Z->params->ppi[Gq]; q++) {
                ...
                for (s = 0; s < Z->params->qpi[Gs]; s++) {
                    ...
                    value = dot_block_sp(T->matrix[Tblock], X, T->params->ppi[Gp], T->params->qpi[Gr], alpha);
                    value_dp = static_cast<double>(value);
                    Z->matrix[Zblock][q][s] += value_dp;
                }
            }
