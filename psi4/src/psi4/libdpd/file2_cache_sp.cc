...
void DPD::file2_cache_close_sp() {
    ...
    while (this_entry != nullptr) {
        ...
        file2_init_sp(&Outfile, this_entry->filenum, this_entry->irrep, this_entry->pnum, this_entry->qnum,
                   this_entry->label);

        next_entry = this_entry->next;

        file2_cache_del_sp(&Outfile);
...
*/
int DPD::file2_cache_add_sp(dpdfile2<float> *File) {
    int h, dpdnum;
    dpd_file2_cache_entry *this_entry;
    ...
    if (this_entry == nullptr) { /* New cache entry */
        ...
        file2_mat_init_sp(File);
        file2_mat_rd_sp(File);
...
int DPD::file2_cache_del_sp(dpdfile2<float> *File) {
    int dpdnum;
    ...
    if (this_entry == nullptr)
        ...
        if (!(this_entry->clean)) file2_mat_wrt_sp(File);
        file2_mat_close_sp(File);
...
void DPD::file2_cache_print_sp(std::string out) {
...
void DPD::file2_cache_dirty_sp(dpdfile2<float> *File) {
    dpd_file2_cache_entry *this_entry;
