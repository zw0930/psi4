...
int DPD::contract244_mp(dpdfile2<float> *X, dpdbuf4<float> *Y, dpdbuf4<double> *Z, int sum_X, int sum_Y, int Ztrans, float alpha, double beta) {
    //dpdbuf4<U> *Z_tmp;
...
#endif
                    newmm_rking_sp(X->matrix[Hx], Xtrans, Ymat[Hy], Ytrans, TMP, numrows[Hz], numlinks[Hx ^ symlink],
                                numcols[Hz], alpha, 0);
                    // Cast the product to double-precision and add onto Zmat
                    ...
                    if (numrows[Hz] && numcols[Hz] && numlinks[Hx ^ symlink]) {
                        if (!Xtrans && !Ytrans) {
                            C_SGEMM('n', 'n', numrows[Hz], numcols[Hz], numlinks[Hx ^ symlink], alpha,
                                    &(X->matrix[Hx][0][0]), numlinks[Hz ^ symlink], &(Ymat[Hy][0][0]), numcols[Hz], 0,
                                    &(TMP[0][0]), numcols[Hz]);
                        } else if (Xtrans && !Ytrans) {
                            ...
                            C_SGEMM('n', 't', numrows[Hz], numcols[Hz], numlinks[Hx ^ symlink], alpha,
                                    ...
                                    numlinks[Hx ^ symlink], 0, &(TMP[0][0]), numcols[Hz]);
                        } else {
                            C_SGEMM('t', 't', numrows[Hz], numcols[Hz], numlinks[Hx ^ symlink], alpha,
                                    &(X->matrix[Hx][0][0]), numrows[Hz], &(Ymat[Hy][0][0]), numlinks[Hx ^ symlink], 0,
                                    &(TMP[0][0]), numcols[Hz]);
                        }
                    }
                    // Cast TMP
  		    ...
  		    // TMP: temporarily holds the product of X and Y in single-precision 
                    ...
                    if (nrows && ncols && nlinks) {
                        C_SGEMM(Xtrans ? 't' : 'n', 'n', nrows, ncols, nlinks, alpha,
                                ...
                                &(Y->matrix[hybuf][0][Y->col_offset[hybuf][GrY]]), ncols, 0,
                                &(TMP[0][0]), ncols);
                    }
                    // Cast and add the product to Z 
                    for ( row = 0; row < nrows; row++ ){
			for ( col = 0; col < ncols; col ++){
			    Z->matrix[hzbuf][row][Z->col_offset[hzbuf][GrZ]+col] = beta *  Z->matrix[hzbuf][row][Z->col_offset[hzbuf][GrZ]+col] + static_cast<double>(TMP[row][col]); 
			}
		    }   
