...
/* Allocates a contiguous block of memory for use as a 2-dimensional array for floats
** ZW 10/2019
*/
...
float **DPD::dpd_block_matrix_sp(size_t n, size_t m) {
    int i;
    float **A, *B;
    ...
    if ((A = (float **)malloc(n * sizeof(float *))) == nullptr) {
    ...
    //#else
    while ((B = (float *)malloc(size * sizeof(float))) == nullptr) {
        //#endif
        /* Priority-based cache */
        if (dpd_main.cachetype == 1) {
            if (file4_cache_del_low_sp()) {
...
void DPD::free_dpd_block_sp(float **array, size_t n, size_t m) {
    size_t size = m * n;
