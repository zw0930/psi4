...
int DPD::file4_mat_irrep_row_rd_sp(dpdfile4_sp *File, int irrep, int row) {
    ...
    if (coltot)
        psio_read(File->filenum, File->label, (char *)File->matrix[irrep][0], coltot * sizeof(float), row_ptr,
                  &next_address);
