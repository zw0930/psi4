...
namespace psi {
int DPD::trans4_mat_irrep_shift31_sp(dpdtrans4<float> *Trans, int buf_block) {
    ...
    for (h_rsp = 0; h_rsp < nirreps; h_rsp++) {
        ...
        Trans->shift.rowtot[buf_block][h_rsp] = rowtot * Trans->buf.params->ppi[h_rsp ^ all_buf_irrep ^ buf_block];
    }

    /* Malloc the pointers to the rows for the shifted access matrix */
    Trans->shift.matrix[buf_block] = (float ***)malloc(nirreps * sizeof(float **));
    for (h = 0; h < nirreps; h++)
        Trans->shift.matrix[buf_block][h] =
            ((!Trans->shift.rowtot[buf_block][h])
                 ? nullptr
                 : (float **)malloc(Trans->shift.rowtot[buf_block][h] * sizeof(float *)));

    /* Calculate the row offsets */
    blocklen = init_int_array(nirreps);
    for (h_rsp = 0; h_rsp < nirreps; h_rsp++)
        blocklen[h_rsp] =
            Trans->buf.params->ppi[h_rsp ^ all_buf_irrep ^ buf_block] * Trans->buf.params->qpi[h_rsp ^ all_buf_irrep];
    ...
    for (pq = 0; pq < Trans->buf.params->coltot[buf_block ^ all_buf_irrep]; pq++) {
        ...
        for (h_pqr = 0; h_pqr < nirreps; h_pqr++) { /* loop over rsp of original dpd*/
            ...
            for (r = 0; (r < Trans->buf.params->ppi[Gr]) && Trans->buf.params->qpi[Gs]; r++) {
                /* Re-assign the row pointer */
                Trans->shift.matrix[buf_block][h_pqr][count[h_pqr]] =
                    &(data[pqcol + rowoff[h_pqr] + (r * Trans->buf.params->qpi[Gs])]);
                count[h_pqr]++;
            }
        }
    }
