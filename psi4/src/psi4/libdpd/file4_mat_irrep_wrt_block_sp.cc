...
int DPD::file4_mat_irrep_wrt_block_sp(dpdfile4_sp *File, int irrep, int start_pq, int num_pq) {
   ...
   // }
    ...
    if (coltot) {
        seek_block = DPD_BIGNUM / (coltot * sizeof(float)); /* no. of rows for which we can compute the address */
        if (seek_block < 1) {
        ...
        for (; start_pq > seek_block; start_pq -= seek_block)
            irrep_ptr = psio_get_address(irrep_ptr, sizeof(float) * seek_block * coltot);
        irrep_ptr = psio_get_address(irrep_ptr, sizeof(float) * start_pq * coltot);
    }

    if (rowtot && coltot)
        psio_write(File->filenum, File->label, (char *)File->matrix[irrep][0], size * ((long)sizeof(float)), irrep_ptr,
                   &next_address);
