...
int DPD::file4_init_sp(dpdfile4_sp *File, int filenum, int irrep, int pqnum, int rsnum, const char *label) {
    int i;
   ...
   // } else {
    ...
    for (i = 1; i < File->params->nirreps; i++) {
        ...
        for (; rowtot > maxrows; rowtot -= maxrows)
            irrep_ptr = psio_get_address(irrep_ptr, sizeof(float) * maxrows * coltot);
        irrep_ptr = psio_get_address(irrep_ptr, sizeof(float) * rowtot * coltot);
