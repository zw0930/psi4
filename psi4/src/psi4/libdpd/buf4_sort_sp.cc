...
int DPD::buf4_sort_sp(dpdbuf4<float> *InBuf, int outfilenum, enum indices index, int pqnum, int rsnum, const char *label) {
    ...
    buf4_init_sp(&OutBuf, outfilenum, my_irrep, pqnum, rsnum, pqnum, rsnum, 0, label);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        if (coltot) {
            ...
            if (maxrows < 1) {
                ...
                dpd_error("buf4_sort_sp_axpy", "outfile");
            }
        } else
            maxrows = DPD_BIGNUM;
    ...
    switch (index) {
        ...
        case pqsr:
            ...
            if (incore) {
                ...
                for (Gpq = 0; Gpq < nirreps; Gpq++) {
                    ...
                    if (!rows_per_bucket) dpd_error("buf4_sort_sp_pqsr: Not enough memory for one row!", "outfile");
                    ...
                    buf4_mat_irrep_init_block_sp(InBuf, Gpq, rows_per_bucket);
                    buf4_mat_irrep_init_block_sp(&OutBuf, Gpq, rows_per_bucket);

                    for (n = 0; n < (rows_left ? nbuckets - 1 : nbuckets); n++) {
                        buf4_mat_irrep_rd_block_sp(InBuf, Gpq, n * rows_per_bucket, rows_per_bucket);
                        ...
                        buf4_mat_irrep_wrt_block_sp(&OutBuf, Gpq, n * rows_per_bucket, rows_per_bucket);
                    }
                    if (rows_left) {
                        buf4_mat_irrep_rd_block_sp(InBuf, Gpq, n * rows_per_bucket, rows_left);
                        ...
                        buf4_mat_irrep_wrt_block_sp(&OutBuf, Gpq, n * rows_per_bucket, rows_left);
                    }

                    buf4_mat_irrep_close_block_sp(InBuf, Gpq, rows_per_bucket);
                    buf4_mat_irrep_close_block_sp(&OutBuf, Gpq, rows_per_bucket);
                }
            }
        ...
        case prqs:
            ...
            if (incore) {
                ...
                for (Gpq = 0; Gpq < nirreps; Gpq++) {
                    ...
                    buf4_mat_irrep_init_block_sp(&OutBuf, Gpq, out_rows_per_bucket);
                    ...
                    for (n = 0; n < (out_rows_left ? out_nbuckets - 1 : out_nbuckets); n++) {
                        ...
                        for (Grow = 0; Grow < nirreps; Grow++) { /*Grow = Gpr*/
                            ...
                            /* allocate space for the bucket of rows */
                            buf4_mat_irrep_init_block_sp(InBuf, Grow, in_rows_per_bucket);

                            /* pqrs <- prqs */
                            for (m = 0; m < (in_rows_left ? in_nbuckets - 1 : in_nbuckets); m++) {
                                in_row_start = m * in_rows_per_bucket;
                                buf4_mat_irrep_rd_block_sp(InBuf, Grow, in_row_start, in_rows_per_bucket);
                            ...
                            if (in_rows_left) {
                                in_row_start = m * in_rows_per_bucket;
                                buf4_mat_irrep_rd_block_sp(InBuf, Grow, in_row_start, in_rows_left);
                            ...
                            buf4_mat_irrep_close_block_sp(InBuf, Grow, in_rows_per_bucket);
                        }
                    ...
                    if (out_rows_left) {
                        ...
                        buf4_mat_irrep_wrt_block_sp(&OutBuf, Gpq, out_row_start, out_rows_left);
                    }

                    buf4_mat_irrep_close_block_sp(&OutBuf, Gpq, out_rows_per_bucket);
                }
            }
        ...
        case psqr:
            ...
            if (incore) {
                ...
                dpd_error("buf4_sort_sp", "outfile");
            }
                ...
                for (Gpq = 0; Gpq < nirreps; Gpq++) {
                    ...
                    for (n = 0; n < (rows_left ? nbuckets - 1 : nbuckets); n++) {
                        ...
                        for (m = 0; m < (rows_left ? nbuckets - 1 : nbuckets); m++) {
                            ...
                            buf4_mat_irrep_rd_block_sp(InBuf, Gpq, in_row_start, rows_per_bucket);
                            for (pq = 0; pq < rows_per_bucket; pq++) {
                        ...
                        if (rows_left) {
                            in_row_start = m * rows_per_bucket;
                            buf4_mat_irrep_rd_block_sp(InBuf, Gpq, in_row_start, rows_left);
                            for (pq = 0; pq < rows_per_bucket; pq++) {
                                /* check to see if this row is contained in the current input-bucket */
                                p = OutBuf.params->roworb[Gpq][pq + out_row_start][0];
                                q = OutBuf.params->roworb[Gpq][pq + out_row_start][1];
                        ...
                        buf4_mat_irrep_wrt_block_sp(&OutBuf, Gpq, out_row_start, rows_per_bucket);
                    ...
                    buf4_mat_irrep_wrt_block_sp(&OutBuf, Gpq, out_row_start, rows_left);
                    buf4_mat_irrep_close_block_sp(InBuf, Gpq, rows_per_bucket);
                    buf4_mat_irrep_close_block_sp(&OutBuf, Gpq, rows_per_bucket);
                } /* Gpq */
            }
        ...
        case rspq:
            ...
            if (incore) {
                ...
                for (Gpq = 0; Gpq < nirreps; Gpq++) {
                    ...
                    buf4_mat_irrep_init_block_sp(&OutBuf, Gpq, out_rows_per_bucket);
                    buf4_mat_irrep_init_block_sp(InBuf, Grs, in_rows_per_bucket);
                    ...
                    for (n = 0; n < out_nbuckets; n++) {
                        ...
                        for (m = 0; m < in_nbuckets; m++) {
                            in_row_start = m * in_rows_per_bucket;
                            buf4_mat_irrep_rd_block_sp(InBuf, Grs, in_row_start,
                        ...
                        buf4_mat_irrep_wrt_block_sp(&OutBuf, Gpq, out_row_start,
                                                 (n == out_nbuckets - 1 ? out_rows_left : out_rows_per_bucket));
                    }

                    buf4_mat_irrep_close_block_sp(&OutBuf, Gpq, out_rows_per_bucket);
                    buf4_mat_irrep_close_block_sp(InBuf, Grs, in_rows_per_bucket);
