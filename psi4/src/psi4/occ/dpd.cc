...
void SymBlockMatrix::add(int h, int i, int j, double value) { matrix_[h][i][j] += value; }  //
...
void SymBlockMatrix::subtract(int h, int i, int j, double value) { matrix_[h][i][j] -= value; }  //
...
void SymBlockMatrix::scale_row(int h, int m, double a) { C_DSCAL(rowspi_[h], a, &(matrix_[h][m][0]), 1); }  //
...
void SymBlockMatrix::set(int h, int i, int j, double value) { matrix_[h][i][j] = value; }  //
...
double *SymBlockMatrix::to_lower_triangle() {
    ...
    if (sizerow != sizecol) return nullptr;
...
void SymBlockMatrix::set_to_identity() {
    ...
    for (int h = 0; h < nirreps_; h++) {
        ...
        if (size) {
            memset(&(matrix_[h][0][0]), 0, size);
            for (int i = 0; i < MIN0(rowspi_[h], colspi_[h]); i++) matrix_[h][i][i] = 1.0;
        }
    }
}  //

void SymBlockMatrix::gemm(bool transa, bool transb, double alpha, const SymBlockMatrix *a, const SymBlockMatrix *b,
                          double beta) {
    char ta = transa ? 't' : 'n';
    char tb = transb ? 't' : 'n';
    ...
    for (int h = 0; h < nirreps_; h++) {
        ...
        if (m && n && k) {
            C_DGEMM(ta, tb, m, n, k, alpha, &(a->matrix_[h][0][0]), nca, &(b->matrix_[h][0][0]), ncb, beta,
                    &(matrix_[h][0][0]), ncc);
        }
    }
}  //
...
void SymBlockMatrix::read(std::shared_ptr<psi::PSIO> psio, int itap, bool readSubBlocks) {
    ...
    if (readSubBlocks == false) {
        ...
        for (int h = 0; h < nirreps_; h++) {
            if (colspi_[h] > 0 && rowspi_[h] > 0)
                psio->read_entry(itap, const_cast<char *>(name_.c_str()), (char *)matrix_[h][0],
                                 sizeof(double) * rowspi_[h] * colspi_[h]);
        }
    }
...
void SymBlockMatrix::read_oooo(std::shared_ptr<psi::PSIO> psio, int itap, int *mosym, int *qt2pitzer, int *occ_off,
                               int *occpi, Array3i *oo_pairidx) {
...
void SymBlockMatrix::read_oovv(std::shared_ptr<psi::PSIO> psio, int itap, int nocc, int *mosym, int *qt2pitzer,
                               int *occ_off, int *vir_off, int *occpi, int *virpi, Array3i *oo_pairidx,
                               Array3i *vv_pairidx) {
    IWL ERIIN(psio.get(), itap, 0.0, 1, 1);
    int ilsti, nbuf, index, fi;
    double value = 0.0;
