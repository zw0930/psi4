...
void d_sort(int reference) {
    ...
    if (reference == 2) { /*** UHF ***/
        ...
        global_dpd_->buf4_close(&D);
        global_dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 11, 10, 11, 10, 0, "D <ij|ab> (ai,jb)");
        ...
        global_dpd_->buf4_close(&D);
        global_dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 11, 10, 11, 10, 0, "D <ij|ab> (aj,ib)");
        ...
        global_dpd_->buf4_close(&D);
        global_dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 11, 10, 11, 10, 0, "D <ij|ab> (bi,ja)");
