    ...
    for (int h = 0; h < d1.params->nirreps; h++) {
        ...
        for (int ij = 0; ij < d2.params->rowtot[h]; ij++) {
            ...
            for (int ab = 0; ab < d2.params->coltot[h]; ab++) {
                ...
                double fbb = fAB.matrix[bsym][B][B];
                d2.matrix[h][ij][ab] =
                    ((A >= (d2.params->rpi[asym] - openpi[asym])) || (B >= (d2.params->spi[bsym] - openpi[bsym]))
                         ? 0.0
                         : 1.0 / (fii + fjj - faa - fbb));
            }
        }
    ...
    for (int h = 0; h < d2.params->nirreps; h++) {
        ...
        for (int ij = 0; ij < d2.params->rowtot[h]; ij++) {
            ...
            for (int ab = 0; ab < d2.params->coltot[h]; ab++) {
                ...
                double fbb = fab.matrix[bsym][B][B];
                d2.matrix[h][ij][ab] =
                    ((I >= (d2.params->ppi[isym] - openpi[isym])) || (J >= (d2.params->qpi[jsym] - openpi[jsym]))
                         ? 0.0
                         : 1.0 / (fii + fjj - faa - fbb));
            }
        }
    ...
    for (int h = 0; h < d2.params->nirreps; h++) {
        ...
        for (int ij = 0; ij < d2.params->rowtot[h]; ij++) {
            ...
            for (int ab = 0; ab < d2.params->coltot[h]; ab++) {
                ...
                double fbb = fab.matrix[bsym][B][B];
                d2.matrix[h][ij][ab] =
                    ((A >= (d2.params->rpi[asym] - openpi[asym])) || (J >= (d2.params->qpi[jsym] - openpi[jsym]))
                         ? 0.0
                         : 1.0 / (fii + fjj - faa - fbb));
            }
        }
