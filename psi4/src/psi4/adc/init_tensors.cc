...
double ADCWfn::rhf_init_tensors() {
    ...
    _ints = new IntegralTransform(reference_wavefunction_, spaces,
                                  IntegralTransform::TransformationType::Restricted);  // printf("madeok\n");
    ...
    global_dpd_->buf4_init(&V, PSIF_LIBTRANS_DPD, 0, ID("[O,V]"), ID("[O,V]"), ID("[O,V]"), ID("[O,V]"), 0,
                           "MO Ints (OV|OV)");
    ...
    for (int h = 0; h < nirrep_; h++) {
        ...
        for (int ij = 0; ij < V.params->rowtot[h]; ij++) {
            ...
            for (int ab = 0; ab < V.params->coltot[h]; ab++) K.matrix[h][ij][ab] = V.matrix[h][ij][ab] / Nij;
        }
        global_dpd_->buf4_mat_irrep_wrt(&K, h);
        global_dpd_->buf4_mat_irrep_close(&K, h);
    ...
    global_dpd_->buf4_close(&K);
    global_dpd_->buf4_init(&K, PSIF_ADC, 0, ID("[O,O]"), ID("[V,V]"), ID("[O,O]"), ID("[V,V]"), 0,
                           "tilde 2 K1234 - K1243");
    ...
    global_dpd_->buf4_init(&V, PSIF_LIBTRANS_DPD, 0, ID("[O,V]"), ID("[O,O]"), ID("[O,V]"), ID("[O>=O]+"), 0,
                           "MO Ints (OV|OO)");
