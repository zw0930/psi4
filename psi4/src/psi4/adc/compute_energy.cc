                ...
                for (int iter = 1; iter <= pole_max_; iter++) {
                    ...
                    if (std::fabs(omega_diff) < conv_) {
                        ...
                        outfile->Printf("->\t%d%3s state   : %10.7f (a.u.), %10.7f (eV)\n", root + 1,
                                        irrep_[irrep].c_str(), omega[root], omega[root] * pc_hartree2ev);
                        outfile->Printf("\tNon-iterative: %10.7f (a.u.), %10.7f (eV)\n", poles_[irrep][root].ps_value,
                                        poles_[irrep][root].ps_value * pc_hartree2ev);
                        outfile->Printf("\t         Occ Vir        Coefficient\n");
                        outfile->Printf("\t---------------------------------------------\n");
                        int nprint;
                        if (nxspi_[irrep] < num_amps_)
                            nprint = nxspi_[irrep];
                        else
                            nprint = num_amps_;
                        ...
                        if (theta > ANGL_TOL_) outfile->Printf("\t#WARNING: Strongly rotated from the CIS state!\n");
                        outfile->Printf("\n");
                        ...
                        /*
                                                ...
                                                for(int sub_irrep = 0;sub_irrep < nirrep_;sub_irrep++){
                                                    ...
                                                    if(row && col){
                                                        ...
                                                        C_DGEMM('n', 't', row, row, col,  -1.0,
                        &(V.matrix[sub_irrep][0][0]), col, &(V.matrix[sub_irrep][0][0]), col, 0.0, &(D[0][0]), row);
                        ...
                        irrep_[sub_irrep]);
                                                ...
                                                outfile->Printf( "]\n");
                                                outfile->Printf( "\tTrace of zeroth-order detachment density-matrix:
                        %7.5e\n", trace);
                                                outfile->Printf( "\n");
                                                free(tracepi);
                                                ...
                                                for(int sub_irrep = 0;sub_irrep < nirrep_;sub_irrep++){
                                                    ...
                                                    if(row && col){
                                                        D   = block_matrix(row, row);               // Detachment
                        density-matrix in MO basis
                                                        Dso = block_matrix(nsopi_[sub_irrep], row); // Detachment
                        density-matrix in SO basis
                                                        ...
                                                        C_DGEMM('t', 'n', row, row, col,  1.0,
                        &(V.matrix[sub_irrep^irrep][0][0]), row, &(V.matrix[sub_irrep^irrep][0][0]), row, 0.0,
                        &(D[0][0]), row);
                                                        C_DSYEV('v', 'u', row, &(D[0][0]), row, pop, work, lwork);
                        //                                C_DGEMM('n', 'n', nsopi_[sub_irrep], row, row, 1.0,
                        &(C[0][0]), row, &(D[0][0]), row, 0.0, &(Dso[0][0]), row);
                                                        for(int i = 0;i < row;i++){
                                                            if(std::fabs(pop[i]) > CUTOFF_DENS_){
                                                                lmax.value = D[0][i];
                                                                lmax.dpdstate = 0;
                        ...
                        irrep_[sub_irrep]);
                                                ...
                                                for(int i = 0;i < nirrep_;i++) outfile->Printf( " %6.4e ", tracepi[i]);
                                                outfile->Printf( "]\n");
                                                outfile->Printf( "\tTrace of zeroth-order attachment density-matrix:
                        %7.5e\n", trace);
                                                outfile->Printf( "\n");
                                                free(tracepi);
                        */
