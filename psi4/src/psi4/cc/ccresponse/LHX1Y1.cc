...
void build_XY(const char *pert_x, int irrep_x, double omega_x, const char *pert_y, int irrep_y, double omega_y) {
        ...
        for (row = 0; row < Z.params->rowtot[h]; row++) {
            ...
            for (col = 0; col < Z.params->coltot[h]; col++) {
                ...
                if (((Isym ^ Esym) == irrep_x) && ((Msym ^ Asym) == irrep_y))
                    Z.matrix[h][row][col] = (X1.matrix[Isym][I][E] * Y1.matrix[Msym][M][A]) +
