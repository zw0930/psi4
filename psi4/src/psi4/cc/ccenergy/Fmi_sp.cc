...
void CCEnergyWavefunction::Fmi_build_sp() {
    dpdfile2<float> FMI, Fmi, FMIt, Fmit, fIJ, fij, fIA, fia;
    dpdfile2<float> tIA, tia, FME, Fme;
    dpdbuf4<float> E_anti, E, D_anti, D;
    dpdbuf4<float> tautIJAB, tautijab, tautIjAb;

    if (params_.ref == 0) { /** RHF **/
        global_dpd_->file2_init_sp(&fIJ, PSIF_CC_OEI, 0, 0, 0, "fIJ_sp");
        global_dpd_->file2_copy_sp(&fIJ, PSIF_CC_OEI, "FMI_sp");
        global_dpd_->file2_close_sp(&fIJ_sp);
    ...
    if (params_.ref == 0) { /** RHF **/
        ...
        global_dpd_->contract44_sp(&D, &tautIjAb, &FMI, 0, 0, 1, 1);
