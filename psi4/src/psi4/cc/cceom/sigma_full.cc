...
void sigma00(int i, int C_irr) {
    ...
    psio_read_entry(PSIF_CC_HBAR, "Reference expectation value", (char *)&(reference_expectation_value),
                    sizeof(double));
        ...
        for (h = 0; h < nirreps; h++) {
            ...
            for (row = 0; row < SIjAb.params->rowtot[h]; row++) {
                ...
                for (col = 0; col < SIjAb.params->coltot[h ^ C_irr]; col++) {
                    ...
                    I = CIA.params->rowidx[i];
                    Isym = CIA.params->psym[i];
                    J = FBJ.params->rowidx[j];
                    Jsym = FBJ.params->psym[j];
                    A = CIA.params->colidx[a];
                    Asym = CIA.params->qsym[a];
                    B = FBJ.params->colidx[b];
                    Bsym = FBJ.params->qsym[b];
