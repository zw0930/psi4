...
/* Computing sigma vectors */
            ...
            for (k = 0; k < eom_params.cs_per_irrep[C_irr]; ++k) {
                ...
                if ((norm > eom_params.residual_tol) || (std::fabs(lambda[k] - lambda_old[k]) > eom_params.eval_tol)) {
                    ...
                    if (params.eom_ref == 0)
                        schmidt_add_RHF(&RIA, &RIjAb, &numCs, C_irr);
                    else
                        schmidt_add(&RIA, &Ria, &RIJAB, &Rijab, &RIjAb, &numCs, C_irr);
                } else {
                    outfile->Printf("%7s\n", "Y");
                    ++num_converged;
                    converged[k] = 1;
            ...
            if ((keep_going == 0) && (iter < eom_params.max_iter)) {
                ...
                if ((params.wfn == "EOM_CC3") && (cc3_stage == 0)) {
                    ...
                    if (cc3_stage == 1)
                        outfile->Printf("Forcing one restart with sigma recomputation.\n");
                    else
                        outfile->Printf("Forcing restart to make sure new sigma vectors give same eigenvalue.\n");
                    outfile->Printf("Collapsing to only %d vector(s).\n", cc3_index + 1);
            ...
            for (i = 0; i < eom_params.cs_per_irrep[C_irr]; ++i) {
                if (converged[i] == 1) {
                    ...
                    if (C_irr == eom_params.prop_sym && i == eom_params.prop_root) {
                        ...
                        //            lambda_old[i]+moinfo.ecc; outfile->Printf("\tPutting into environment CURRENT
