...
void read_guess_init() {
    ...
    for (i = 0; i < moinfo.nirreps; ++i) outfile->Printf("%s %d, ", moinfo.irr_labs[i], eom_params.states_per_irrep[i]);
    outfile->Printf("\n");
    outfile->Printf("and Rs_per_irrep: \n\t");
    for (i = 0; i < moinfo.nirreps; ++i) outfile->Printf("%s %d, ", moinfo.irr_labs[i], eom_params.cs_per_irrep[i]);
