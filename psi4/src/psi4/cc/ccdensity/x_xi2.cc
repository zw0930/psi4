    ...
    if (params.ref != 0) { /* ROHF or UHF for XIJAB */
        ...
        for (h = 0; h < nirreps; h++) {
            ...
            for (row = 0; row < Z.params->rowtot[h]; row++) {
                ...
                I = FME.params->rowidx[i];
                Isym = FME.params->psym[i];
                J = IIA.params->rowidx[j];
                Jsym = IIA.params->psym[j];
                for (col = 0; col < Z.params->coltot[h ^ G_irr]; col++) {
                    a = Z.params->colorb[h ^ G_irr][col][0];
                    b = Z.params->colorb[h ^ G_irr][col][1];
                    A = FME.params->colidx[a];
                    Asym = FME.params->qsym[a];
                    B = IIA.params->colidx[b];
                    Bsym = IIA.params->qsym[b];
    ...
    if (params.ref == 0) { /* RHF XIjAb */
        ...
        for (h = 0; h < nirreps; h++) {
            ...
            for (row = 0; row < XIjAb.params->rowtot[h]; row++) {
                ...
                J = IIA.params->rowidx[j];
                Jsym = IIA.params->psym[j];
                II = IIA.params->rowidx[i];
                IIsym = IIA.params->psym[i];
                JJ = FME.params->rowidx[j];
                JJsym = FME.params->psym[j];
                for (col = 0; col < XIjAb.params->coltot[h ^ G_irr]; col++) {
                    ...
                    B = IIA.params->colidx[b];
                    Bsym = IIA.params->qsym[b];
                    AA = IIA.params->colidx[a];
                    AAsym = IIA.params->qsym[a];
                    BB = FME.params->colidx[b];
                    BBsym = FME.params->qsym[b];
                    if ((Isym == Asym) && ((Jsym ^ Bsym) == G_irr))
