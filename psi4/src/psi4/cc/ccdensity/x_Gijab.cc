...
void x_Gijab_rohf() {
    ...
    if (G_irr == 0) {
        ...
        global_dpd_->buf4_scm(&G, 0.0);
        global_dpd_->buf4_close(&G);
        global_dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 0, 5, 0, 5, 0, "GIjAb");
        global_dpd_->buf4_scm(&G, 0.0);
        global_dpd_->buf4_close(&G);
    }
    ...
    global_dpd_->file2_close(&Z1);
    global_dpd_->buf4_close(&T);
    global_dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, qpsr, 0, 5, "Z(Ij,Ab)");
    global_dpd_->buf4_close(&Z);
    ...
    global_dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "tauIjbA");
    global_dpd_->file2_init(&I1, PSIF_EOM_TMP, G_irr, 1, 1, "LR_vv");
    global_dpd_->contract244(&I1, &T, &Z, 0, 2, 1, 1.0, 0.0);
    global_dpd_->file2_close(&I1);
    global_dpd_->buf4_close(&T);
    global_dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, pqsr, 0, 5, "Z(Ij,Ab)");
    global_dpd_->buf4_close(&Z);
    global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "Z(Ij,Ab)");
    global_dpd_->buf4_axpy(&Z, &G, -1.0);
    global_dpd_->buf4_close(&G);
    ...
    global_dpd_->buf4_close(&G);
    global_dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 0, 5, 0, 5, 0, "GIjAb");
    global_dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 0, 0, 0, 0, 0, "Tau2L2_OoOo");
    ...
    global_dpd_->buf4_init(&I, PSIF_EOM_TMP, G_irr, 2, 10, 2, 10, 0, "L2R1_OOVO(pqsr)");
    global_dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
    ...
    global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 0, 0, 0, 0, "Z(Ij,Mn)");
    global_dpd_->buf4_init(&I, PSIF_EOM_TMP, G_irr, 0, 10, 0, 10, 0, "L2R1_OovO(pqsr)");
    global_dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tia");
    global_dpd_->contract424(&I, &T1, &Z, 3, 1, 1, 1.0, 0.0);
    global_dpd_->file2_close(&T1);
    global_dpd_->buf4_close(&I);
    global_dpd_->buf4_init(&I, PSIF_EOM_TMP, G_irr, 0, 11, 0, 11, 0, "L2R1_OoVo");
    global_dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
    global_dpd_->contract244(&T1, &I, &Z, 1, 2, 0, 1.0, 1.0);
    global_dpd_->file2_close(&T1);
    global_dpd_->buf4_close(&I);
    /* add terms 12, 14 */
    global_dpd_->buf4_init(&I, PSIF_EOM_TMP, G_irr, 0, 0, 0, 0, 0, "R2L2_OoOo");
    ...
    global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 2, 11, 2, 11, 0, "Z(I>J,AN)");
    ...
    global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 11, 0, 11, 0, "Z(Ij,An)");
    global_dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 0, 0, 0, 0, 0, "Tau2L2_OoOo");
    global_dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
    global_dpd_->contract244(&R1, &I, &Z, 0, 2, 1, 1.0, 0.0);
    global_dpd_->file2_close(&R1);
    ...
    global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 0, 11, 0, 11, 0, "Z2(Ij,An)");
    global_dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 0, 0, 0, 0, 0, "Tau2L2_OoOo");
    global_dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
    ...
    global_dpd_->file2_close(&I1);
    global_dpd_->buf4_close(&R);
    global_dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, qprs, 0, 5, "Z(Ij,Ab)");
    global_dpd_->buf4_close(&Z);
    global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "Z(Ij,Ab)");
    global_dpd_->buf4_axpy(&Z, &G, -1.0);
    global_dpd_->buf4_close(&Z);
    ...
    if (!params.connect_xi) {
        ...
        global_dpd_->buf4_close(&Z);
        global_dpd_->buf4_close(&G);
        global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 10, 0, 10, 0, "Z(Ij,Nb)");
        global_dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "tIjAb");
        global_dpd_->file2_init(&I1, PSIF_EOM_TMP, G_irr, 0, 1, "L2R1_OV");
    ...
    global_dpd_->buf4_init(&Z, PSIF_EOM_TMP0, G_irr, 10, 10, 10, 10, 0, "Z(IA,JB)");
    global_dpd_->buf4_init(&I, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_OVOV");
    ...
    global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "Z(Ib,jA)");
    global_dpd_->buf4_init(&I, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_oVoV");
    ...
    global_dpd_->buf4_close(&I);
    global_dpd_->buf4_close(&T);
    global_dpd_->buf4_sort_axpy(&Z, PSIF_EOM_TMP0, psrq, 10, 10, "Z(IA,jb)", 1.0);
    global_dpd_->buf4_close(&Z);
    ...
    global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 10, 0, 10, 0, 0, "Z(JB,NI)");
    global_dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 10, 10, 10, 10, 0, "VIAJB");
    global_dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
    global_dpd_->contract424(&I, &T1, &Z, 3, 1, 0, 1.0, 0.0);
    global_dpd_->file2_close(&T1);
    global_dpd_->buf4_close(&I);
    global_dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 11, 10, 11, 10, 0, "Z(AI,JB)");
    global_dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
    global_dpd_->contract244(&R1, &Z, &Z2, 0, 2, 0, 1.0, 0.0);
    global_dpd_->file2_close(&R1);
    global_dpd_->buf4_close(&Z);
    global_dpd_->buf4_sort_axpy(&Z2, PSIF_EOM_TMP0, qprs, 10, 10, "Z(IA,JB)", -1.0);
    global_dpd_->buf4_close(&Z2);
    ...
    global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 10, 0, 10, 0, 0, "Z(Ib,Nj)");
    global_dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 10, 10, 10, 10, 0, "VIaJb");
    global_dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tia");
    global_dpd_->contract424(&I, &T1, &Z, 3, 1, 0, 1.0, 0.0);
    global_dpd_->file2_close(&T1);
    global_dpd_->buf4_close(&I);
    global_dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 10, 11, 10, 11, 0, "Z(Ib,Aj)");
    global_dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
    global_dpd_->contract244(&R1, &Z, &Z2, 0, 2, 1, 1.0, 0.0);
    global_dpd_->file2_close(&R1);
    global_dpd_->buf4_close(&Z);
    global_dpd_->buf4_sort_axpy(&Z2, PSIF_EOM_TMP0, prsq, 10, 10, "Z(IA,jb)", +1.0);
    global_dpd_->buf4_close(&Z2);

    psio_close(PSIF_EOM_TMP1, 0);
    psio_open(PSIF_EOM_TMP1, PSIO_OPEN_NEW);

    global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 10, 0, 10, 0, 0, "Z(jA,nI)");
    global_dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 10, 10, 10, 10, 0, "ViAjB");
    global_dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
    global_dpd_->contract424(&I, &T1, &Z, 3, 1, 0, 1.0, 0.0);
    global_dpd_->file2_close(&T1);
    global_dpd_->buf4_close(&I);
    global_dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 11, 10, 11, 10, 0, "Z(bI,jA)");
    global_dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "Ria");
    global_dpd_->contract244(&R1, &Z, &Z2, 0, 2, 0, 1.0, 0.0);
    global_dpd_->file2_close(&R1);
    global_dpd_->buf4_close(&Z);
    global_dpd_->buf4_sort(&Z2, PSIF_EOM_TMP1, sqrp, 11, 10, "Z(AI,jb)");
    global_dpd_->buf4_close(&Z2);
    global_dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 11, 10, 11, 10, 0, "Z(AI,jb)");
    global_dpd_->buf4_sort_axpy(&Z2, PSIF_EOM_TMP0, qprs, 10, 10, "Z(IA,jb)", +1.0);
    global_dpd_->buf4_close(&Z2);

    global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 10, 0, 10, 0, 0, "Z(IA,nj)");
    global_dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 10, 10, 10, 10, 0, "VIAjb");
    global_dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tia");
    global_dpd_->contract424(&I, &T1, &Z, 3, 1, 0, 1.0, 0.0);
    global_dpd_->file2_close(&T1);
    global_dpd_->buf4_close(&I);
    global_dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 10, 11, 10, 11, 0, "Z(IA,bj)");
    global_dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "Ria");
    global_dpd_->contract244(&R1, &Z, &Z2, 0, 2, 1, 1.0, 0.0);
    global_dpd_->file2_close(&R1);
    global_dpd_->buf4_close(&Z);
    global_dpd_->buf4_sort_axpy(&Z2, PSIF_EOM_TMP0, pqsr, 10, 10, "Z(IA,jb)", -1.0);
    global_dpd_->buf4_close(&Z2);
    ...
    global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 0, 10, 0, 0, "Z(JB,NI)");
    global_dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 10, 10, 10, 10, 0, "VIAJB");
    global_dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
    ...
    global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 0, 10, 0, 0, "Z(Ib,Nj)");
    global_dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 10, 10, 10, 10, 0, "VIaJb");
    global_dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "Ria");
    global_dpd_->contract424(&I, &R1, &Z, 3, 1, 0, 1.0, 0.0);
    global_dpd_->file2_close(&R1);
    ...
    global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 0, 10, 0, 0, "Z(jA,nI)");
    global_dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 10, 10, 10, 10, 0, "ViAjB");
    global_dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
    global_dpd_->contract424(&I, &R1, &Z, 3, 1, 0, 1.0, 0.0);
    global_dpd_->file2_close(&R1);
    ...
    global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 0, 10, 0, 0, "Z(IA,nj)");
    global_dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 10, 10, 10, 10, 0, "VIAjb");
    global_dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "Ria");
    global_dpd_->contract424(&I, &R1, &Z, 3, 1, 0, 1.0, 0.0);
    global_dpd_->file2_close(&R1);
    ...
    global_dpd_->buf4_init(&Z2, PSIF_EOM_TMP0, G_irr, 10, 10, 10, 10, 0, "Z(IA,JB)");
    ...
    global_dpd_->buf4_init(&Z, PSIF_EOM_TMP0, G_irr, 10, 10, 10, 10, 0, "Z(IA,jb)");
    global_dpd_->buf4_sort_axpy(&Z, PSIF_EOM_TMP0, prqs, 0, 5, "GIjAb", 1.0);
    global_dpd_->buf4_close(&Z);
    ...
    global_dpd_->buf4_close(&G);
    global_dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 0, 5, 0, 5, 0, "GIjAb");
    global_dpd_->buf4_init(&V, PSIF_CC_GAMMA, G_irr, 0, 5, 0, 5, 0, "GIjAb");
    global_dpd_->buf4_axpy(&G, &V, 0.5);
    global_dpd_->buf4_close(&V);
    global_dpd_->buf4_close(&G);
...
void x_Gijab_rohf_2() {
    ...
    global_dpd_->file2_init(&L1R2B, PSIF_EOM_TMP, G_irr, 0, 1, "L1R2_ov");
    ...
    global_dpd_->file2_init(&Z1B, PSIF_EOM_TMP1, G_irr, 0, 1, "Zia");
    ...
    global_dpd_->file2_init(&I1B, PSIF_EOM_TMP, L_irr, 1, 1, "LT2_vv");
    ...
    global_dpd_->file2_close(&R1A);
    global_dpd_->file2_close(&I1A);
    global_dpd_->file2_init(&I1B, PSIF_EOM_TMP, L_irr, 0, 0, "LT2_oo");
    global_dpd_->file2_init(&R1B, PSIF_CC_GR, R_irr, 0, 1, "Ria");
    ...
    global_dpd_->file2_mat_init(&Z1A);
    global_dpd_->file2_mat_init(&Z1B);
    global_dpd_->file2_mat_rd(&Z1A);
    global_dpd_->file2_mat_rd(&Z1B);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            ...
            I1 = Z1A.params->rowidx[i];
            I1sym = Z1A.params->psym[i];
            I2 = I1;
            I2sym = I1sym;
            I3 = T1A.params->rowidx[i];
            I3sym = T1A.params->psym[i];
            I4 = I3;
            I4sym = I3sym;
            J1 = T1A.params->rowidx[j];
            J1sym = T1A.params->psym[j];
            J2 = J1;
            J2sym = J1sym;
            J3 = Z1A.params->rowidx[j];
            J3sym = Z1A.params->psym[j];
            J4 = J3;
            J4sym = J3sym;
            for (col = 0; col < G.params->coltot[h]; col++) {
                a = G.params->colorb[h][col][0];
                b = G.params->colorb[h][col][1];
                A1 = Z1A.params->colidx[a];
                A1sym = Z1A.params->qsym[a];
                A4 = A1;
                A4sym = A1sym;
                A2 = T1A.params->colidx[a];
                A2sym = T1A.params->qsym[a];
                A3 = A2;
                A3sym = A2sym;
                B1 = T1A.params->colidx[b];
                B1sym = T1A.params->qsym[b];
                B4 = B1;
                B4sym = B1sym;
                B2 = Z1A.params->colidx[b];
                B2sym = Z1A.params->qsym[b];
                B3 = B2;
                B3sym = B2sym;
                /* + Z(I,A) T(J,B) */
                if (((I1sym ^ A1sym) == G_irr) && (J1sym == B1sym))
                    G.matrix[h][row][col] += Z1A.matrix[I1sym][I1][A1] * T1A.matrix[J1sym][J1][B1];
                /* - Z(I,B) T(J,A) */
                if (((I2sym ^ B2sym) == G_irr) && (J2sym == A2sym))
                    G.matrix[h][row][col] -= Z1A.matrix[I2sym][I2][B2] * T1A.matrix[J2sym][J2][A2];
                /* + T(I,A) Z(J,B) */
                if (((J3sym ^ B3sym) == G_irr) && (I3sym == A3sym))
                    G.matrix[h][row][col] += Z1A.matrix[J3sym][J3][B3] * T1A.matrix[I3sym][I3][A3];
                /* - T(I,B) Z(J,A) */
                if (((J4sym ^ A4sym) == G_irr) && (I4sym == B4sym))
                    G.matrix[h][row][col] -= Z1A.matrix[J4sym][J4][A4] * T1A.matrix[I4sym][I4][B4];
            }
        }
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            i = G.params->roworb[h][row][0];
            j = G.params->roworb[h][row][1];
            I1 = Z1B.params->rowidx[i];
            I1sym = Z1B.params->psym[i];
            I2 = I1;
            I2sym = I1sym;
            I3 = T1B.params->rowidx[i];
            I3sym = T1B.params->psym[i];
            I4 = I3;
            I4sym = I3sym;
            J1 = T1B.params->rowidx[j];
            J1sym = T1B.params->psym[j];
            J2 = J1;
            J2sym = J1sym;
            J3 = Z1B.params->rowidx[j];
            J3sym = Z1B.params->psym[j];
            J4 = J3;
            J4sym = J3sym;
            for (col = 0; col < G.params->coltot[h]; col++) {
                a = G.params->colorb[h][col][0];
                b = G.params->colorb[h][col][1];
                A1 = Z1B.params->colidx[a];
                A1sym = Z1B.params->qsym[a];
                A4 = A1;
                A4sym = A1sym;
                A2 = T1B.params->colidx[a];
                A2sym = T1B.params->qsym[a];
                A3 = A2;
                A3sym = A2sym;
                B1 = T1B.params->colidx[b];
                B1sym = T1B.params->qsym[b];
                B4 = B1;
                B4sym = B1sym;
                B2 = Z1B.params->colidx[b];
                B2sym = Z1B.params->qsym[b];
                B3 = B2;
                B3sym = B2sym;
                /* + Z(i,a) T(j,b) */
                if (((I1sym ^ A1sym) == G_irr) && (J1sym == B1sym))
                    G.matrix[h][row][col] += Z1B.matrix[I1sym][I1][A1] * T1B.matrix[J1sym][J1][B1];
                /* - Z(i,b) T(j,a) */
                if (((I2sym ^ B2sym) == G_irr) && (J2sym == A2sym))
                    G.matrix[h][row][col] -= Z1B.matrix[I2sym][I2][B2] * T1B.matrix[J2sym][J2][A2];
                /* + T(i,a) Z(j,b) */
                if (((J3sym ^ B3sym) == G_irr) && (I3sym == A3sym))
                    G.matrix[h][row][col] += Z1B.matrix[J3sym][J3][B3] * T1B.matrix[I3sym][I3][A3];
                /* - T(i,b) Z(j,a) */
                if (((J4sym ^ A4sym) == G_irr) && (I4sym == B4sym))
                    G.matrix[h][row][col] -= Z1B.matrix[J4sym][J4][A4] * T1B.matrix[I4sym][I4][B4];
            }
        }
        global_dpd_->buf4_mat_irrep_wrt(&G, h);
        global_dpd_->buf4_mat_irrep_close(&G, h);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            ...
            I1 = Z1A.params->rowidx[i];
            I1sym = Z1A.params->psym[i];
            I2 = T1A.params->rowidx[i];
            I2sym = T1A.params->psym[i];
            J1 = T1B.params->rowidx[j];
            J1sym = T1B.params->psym[j];
            J2 = Z1B.params->rowidx[j];
            J2sym = Z1B.params->psym[j];
            for (col = 0; col < G.params->coltot[h]; col++) {
                a = G.params->colorb[h][col][0];
                b = G.params->colorb[h][col][1];
                A1 = Z1A.params->colidx[a];
                ...
                /* + Z(I,A) T(j,b) */
                if (((I1sym ^ A1sym) == G_irr) && (J1sym == B1sym))
                    G.matrix[h][row][col] += Z1A.matrix[I1sym][I1][A1] * T1B.matrix[J1sym][J1][B1];
                /* + T(I,A) Z(j,b) */
                if (((J2sym ^ B2sym) == G_irr) && (I2sym == A2sym))
                    G.matrix[h][row][col] += T1A.matrix[I2sym][I2][A2] * Z1B.matrix[J2sym][J2][B2];
            }
        }
        global_dpd_->buf4_mat_irrep_wrt(&G, h);
        global_dpd_->buf4_mat_irrep_close(&G, h);
    ...
    global_dpd_->file2_mat_close(&Z1A);
    global_dpd_->file2_mat_close(&Z1B);
...
void x_Gijab_rohf_3() {
    ...
    global_dpd_->file2_init(&L1T2B, PSIF_EOM_TMP, L_irr, 0, 1, "L1T2_ov");
    ...
    global_dpd_->file2_init(&Z1B, PSIF_EOM_TMP1, L_irr, 0, 1, "Zia");
    ...
    global_dpd_->file2_close(&T1A);
    global_dpd_->file2_close(&I1A);
    global_dpd_->file2_init(&I1B, PSIF_EOM_TMP, L_irr, 1, 1, "LT1_vv");
    global_dpd_->file2_init(&T1B, PSIF_CC_OEI, 0, 0, 1, "tia");
    global_dpd_->contract222(&T1B, &I1B, &Z1B, 0, 1, -1.0, 1.0);
    global_dpd_->file2_close(&T1B);
    global_dpd_->file2_close(&I1B);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            ...
            I2 = I1;
            I2sym = I1sym;
            I3 = R1A.params->rowidx[i];
            I3sym = R1A.params->psym[i];
            I4 = I3;
            I4sym = I3sym;
            J1 = R1A.params->rowidx[j];
            J1sym = R1A.params->psym[j];
            J2 = J1;
            J2sym = J1sym;
            J3 = Z1A.params->rowidx[j];
            J3sym = Z1A.params->psym[j];
            ...
            for (col = 0; col < G.params->coltot[h]; col++) {
                ...
                A4 = A1;
                A4sym = A1sym;
                A2 = R1A.params->colidx[a];
                A2sym = R1A.params->qsym[a];
                A3 = A2;
                A3sym = A2sym;
                B1 = R1A.params->colidx[b];
                B1sym = R1A.params->qsym[b];
                B4 = B1;
                B4sym = B1sym;
                B2 = Z1A.params->colidx[b];
                B2sym = Z1A.params->qsym[b];
                ...
                if (((I1sym ^ A1sym) == L_irr) && ((J1sym ^ B1sym) == R_irr))
                    G.matrix[h][row][col] += Z1A.matrix[I1sym][I1][A1] * R1A.matrix[J1sym][J1][B1];
                /* - Z(I,B) R(J,A) */
                if (((I2sym ^ B2sym) == L_irr) && ((J2sym ^ A2sym) == R_irr))
                    G.matrix[h][row][col] -= Z1A.matrix[I2sym][I2][B2] * R1A.matrix[J2sym][J2][A2];
                /* + R(I,A) Z(J,B) */
                if (((J3sym ^ B3sym) == L_irr) && ((I3sym ^ A3sym) == R_irr))
                    G.matrix[h][row][col] += Z1A.matrix[J3sym][J3][B3] * R1A.matrix[I3sym][I3][A3];
                /* - R(I,B) Z(J,A) */
                if (((J4sym ^ A4sym) == L_irr) && ((I4sym ^ B4sym) == R_irr))
                    G.matrix[h][row][col] -= Z1A.matrix[J4sym][J4][A4] * R1A.matrix[I4sym][I4][B4];
            }
        }
        global_dpd_->buf4_mat_irrep_wrt(&G, h);
        global_dpd_->buf4_mat_irrep_close(&G, h);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            ...
            I2 = I1;
            I2sym = I1sym;
            I3 = R1B.params->rowidx[i];
            I3sym = R1B.params->psym[i];
            I4 = I3;
            I4sym = I3sym;
            J1 = R1B.params->rowidx[j];
            J1sym = R1B.params->psym[j];
            J2 = J1;
            J2sym = J1sym;
            J3 = Z1B.params->rowidx[j];
            J3sym = Z1B.params->psym[j];
            ...
            for (col = 0; col < G.params->coltot[h]; col++) {
                ...
                A4 = A1;
                A4sym = A1sym;
                A2 = R1B.params->colidx[a];
                A2sym = R1B.params->qsym[a];
                A3 = A2;
                A3sym = A2sym;
                B1 = R1B.params->colidx[b];
                B1sym = R1B.params->qsym[b];
                B4 = B1;
                B4sym = B1sym;
                B2 = Z1B.params->colidx[b];
                B2sym = Z1B.params->qsym[b];
                ...
                /* + Z(i,a) R(j,b) */
                if (((I1sym ^ A1sym) == L_irr) && ((J1sym ^ B1sym) == R_irr))
                    G.matrix[h][row][col] += Z1B.matrix[I1sym][I1][A1] * R1B.matrix[J1sym][J1][B1];
                /* - Z(i,b) R(j,a) */
                if (((I2sym ^ B2sym) == L_irr) && ((J2sym ^ A2sym) == R_irr))
                    G.matrix[h][row][col] -= Z1B.matrix[I2sym][I2][B2] * R1B.matrix[J2sym][J2][A2];
                /* + R(i,a) Z(j,b) */
                if (((J3sym ^ B3sym) == L_irr) && ((I3sym ^ A3sym) == R_irr))
                    G.matrix[h][row][col] += Z1B.matrix[J3sym][J3][B3] * R1B.matrix[I3sym][I3][A3];
                /* - R(i,b) Z(j,a) */
                if (((J4sym ^ A4sym) == L_irr) && ((I4sym ^ B4sym) == R_irr))
                    G.matrix[h][row][col] -= Z1B.matrix[J4sym][J4][A4] * R1B.matrix[I4sym][I4][B4];
            }
        }
        global_dpd_->buf4_mat_irrep_wrt(&G, h);
        global_dpd_->buf4_mat_irrep_close(&G, h);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            ...
            I1 = Z1A.params->rowidx[i];
            I1sym = Z1A.params->psym[i];
            I2 = R1A.params->rowidx[i];
            I2sym = R1A.params->psym[i];
            J1 = R1B.params->rowidx[j];
            J1sym = R1B.params->psym[j];
            J2 = Z1B.params->rowidx[j];
            J2sym = Z1B.params->psym[j];
            for (col = 0; col < G.params->coltot[h]; col++) {
                ...
                /* + Z(I,A) R(j,b) */
                if (((I1sym ^ A1sym) == L_irr) && ((J1sym ^ B1sym) == R_irr))
                    G.matrix[h][row][col] += Z1A.matrix[I1sym][I1][A1] * R1B.matrix[J1sym][J1][B1];
                /* + R(I,A) Z(j,b) */
                if (((J2sym ^ B2sym) == L_irr) && ((I2sym ^ A2sym) == R_irr))
                    G.matrix[h][row][col] += R1A.matrix[I2sym][I2][A2] * Z1B.matrix[J2sym][J2][B2];
            }
        }
        global_dpd_->buf4_mat_irrep_wrt(&G, h);
        global_dpd_->buf4_mat_irrep_close(&G, h);
