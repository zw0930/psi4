...
void x_Gijka_6_rohf() {
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            i = G.params->roworb[h][row][0];
            j = G.params->roworb[h][row][1];
            I = LR1B.params->colidx[i];
            Isym = LR1B.params->qsym[i];
            J = T1B.params->rowidx[j];
            Jsym = T1B.params->psym[j];
            II = T1B.params->rowidx[i];
            IIsym = T1B.params->psym[i];
            JJ = LR1B.params->colidx[j];
            JJsym = LR1B.params->qsym[j];
            for (col = 0; col < G.params->coltot[h ^ G_irr]; col++) {
                k = G.params->colorb[h ^ G_irr][col][0];
                a = G.params->colorb[h ^ G_irr][col][1];
                K = LR1B.params->rowidx[k];
                Ksym = LR1B.params->psym[k];
                A = T1B.params->colidx[a];
                Asym = T1B.params->qsym[a];
                if (((Ksym ^ Isym) == G_irr) && (Jsym == Asym))
                    G.matrix[h][row][col] -= LR1B.matrix[Ksym][K][I] * T1B.matrix[Jsym][J][A];
                if (((Ksym ^ JJsym) == G_irr) && (IIsym == Asym))
                    G.matrix[h][row][col] += LR1B.matrix[Ksym][K][JJ] * T1B.matrix[IIsym][II][A];
            }
        }
        global_dpd_->buf4_mat_irrep_wrt(&G, h);
        global_dpd_->buf4_mat_irrep_close(&G, h);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            ...
            for (col = 0; col < G.params->coltot[h ^ G_irr]; col++) {
                ...
                Asym = T1B.params->qsym[a];
                if (((Ksym ^ Isym) == G_irr) && (Jsym == Asym))
                    G.matrix[h][row][col] -= LR1A.matrix[Ksym][K][I] * T1B.matrix[Jsym][J][A];
            }
        }
        global_dpd_->buf4_mat_irrep_wrt(&G, h);
        global_dpd_->buf4_mat_irrep_close(&G, h);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            ...
            for (col = 0; col < G.params->coltot[h ^ G_irr]; col++) {
                ...
                Asym = T1A.params->qsym[a];
                if (((Ksym ^ Isym) == G_irr) && (Jsym == Asym))
                    G.matrix[h][row][col] -= LR1B.matrix[Ksym][K][I] * T1A.matrix[Jsym][J][A];
            }
        }
        global_dpd_->buf4_mat_irrep_wrt(&G, h);
        global_dpd_->buf4_mat_irrep_close(&G, h);
