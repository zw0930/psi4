...
void build_Z_ROHF() {
    ...
    num_ai = 0;
    for (h = 0; h < nirreps; h++) num_ai += virtpi[h] * occpi[h];
    ...
    for (h = 0, count = 0, rank = 0; h < nirreps; h++)
        for (a = 0; a < virtpi[h]; a++)
            for (i = 0; i < occpi[h]; i++) {
                if ((a >= (virtpi[h] - openpi[h])) && (i >= (occpi[h] - openpi[h])))
                    T[count][count] = 0.0;
                else {
                    T[count][count] = 1.0;
                    rank++;
    ...
    num_ai = 0;
    for (h = 0; h < nirreps; h++) num_ai += X1.params->rowtot[h] * X1.params->coltot[h];

    Z = block_matrix(1, num_ai);
    for (h = 0, count = 0; h < nirreps; h++)
        for (a = 0; a < X1.params->rowtot[h]; a++)
            for (i = 0; i < X1.params->coltot[h]; i++) Z[0][count++] = -X1.matrix[h][a][i];
    ...
    if (num_ai) C_DGEMV('n', num_ai, num_ai, 1.0, &(T[0][0]), num_ai, &(Z[0][0]), 1, 0.0, &(X[0][0]), 1);
    ...
    if (num_ai)
        C_DGEMM('n', 'n', num_ai, num_ai, num_ai, 1.0, &(A.matrix[0][0][0]), num_ai, &(T[0][0]), num_ai, 0.0,
                &(Y[0][0]), num_ai);
    /*  newmm(T,0,Y,0,A.matrix[0],num_ai,num_ai,num_ai,1.0,0.0); */
    if (num_ai)
        C_DGEMM('n', 'n', num_ai, num_ai, num_ai, 1.0, &(T[0][0]), num_ai, &(Y[0][0]), num_ai, 0.0,
                &(A.matrix[0][0][0]), num_ai);
    free_block(Y);
    ...
    if (num_ai) C_DGEMV('n', num_ai, num_ai, 1.0, &(T[0][0]), num_ai, &(X[0][0]), 1, 0.0, &(Z[0][0]), 1);
    free_block(X);
