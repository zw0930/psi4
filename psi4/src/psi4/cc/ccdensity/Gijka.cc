            ...
            for (h = 0; h < nirreps; h++) {
                ...
                for (row = 0; row < G.params->rowtot[h]; row++) {
                    ...
                    for (col = 0; col < G.params->coltot[h]; col++) {
                        ...
                        Isym = g.params->psym[i];
                        Jsym = T1.params->psym[j];
                        K = g.params->colidx[k];
                        A = T1.params->colidx[a];
                        Ksym = g.params->qsym[k];
                        Asym = T1.params->qsym[a];

                        if ((Isym == Ksym) && (Jsym == Asym)) value += g.matrix[Isym][I][K] * T1.matrix[Jsym][J][A];
            ...
            for (h = 0; h < nirreps; h++) {
                ...
                for (row = 0; row < G.params->rowtot[h]; row++) {
                    ...
                    for (col = 0; col < G.params->coltot[h]; col++) {
                        ...
                        Jsym = g.params->psym[j];
                        Isym = T1.params->psym[i];

                        if ((Jsym == Ksym) && (Isym == Asym)) value -= g.matrix[Jsym][J][K] * T1.matrix[Isym][I][A];
